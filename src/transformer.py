...
import numpy as np
from tensorflow.keras.layers import Input, Dense, GRU, Embedding, Multiply, Attention, Reshape, Lambda
...
encoder_layer = TransformerEncoderLayer(d_model=32, num_heads=8, dff=2048)
x_s = encoder_layer(x_s)
x_s = Lambda(lambda x: k.sum(x, axis=1))(x_s)
x_s = Dense(DIM_LATENT)(x_s)
