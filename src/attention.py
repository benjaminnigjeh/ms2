...
import numpy as np
from tensorflow.keras.layers import Input, Dense, GRU, Embedding, Multiply, Reshape, Permute, RepeatVector, Lambda 
...
gru = GRU(DIM_LATENT, return_sequences=True)(x_s)

# Attention mechanism
attention = Dense(1, activation='tanh')(gru)
attention = Reshape((-1,))(attention)
attention = Lambda(lambda x: k.softmax(x))(attention)
attention = RepeatVector(124)(attention)
attention = Permute([2, 1])(attention)

# Apply the attention weights
sent_representation = Multiply()([gru, attention])
x_s = Lambda(lambda x: k.sum(x, axis=1))(sent_representation)
...
def main():
  if True:
    ...
    model.save('model_20Epoch_attention.keras')
